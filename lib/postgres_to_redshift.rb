...
class PostgresToRedshift
  ...
  def column_definitions(table)
    column_command = <<-SQL
      ...
      WHERE table_schema = '#{PostgresToRedshift.source_schema}' AND table_name='#{table.name}' AND (LOWER(column_name) NOT LIKE '%password%' OR LOWER(column_name) NOT LIKE '%token%')
