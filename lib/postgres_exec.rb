...
class PostgresExec
  def self.exec_sql
    ...
    if (PostgresExec.slack_on_success == 'true')
      message = "[REXE]SUCCESS: Script executed on Postgres | BUCKET: #{PostgresExec.bucket_name} | SCRIPT: #{PostgresExec.object_name}"
      SLACK_NOTIFIER.ping message
    end
  rescue => e
    SLACK_NOTIFIER.ping "[REXE]#{e.message.gsub("\r"," ").gsub("\n"," ")} | BUCKET: #{PostgresExec.bucket_name} | SCRIPT: #{PostgresExec.object_name}"
  end

  def self.target_uri
    @target_uri ||= URI.parse(ENV['PG_TARGET_URI'])
  end
