...
POSTGRES_TO_REDSHIFT_S3_DATABASE_EXPORT_BUCKET=$(aws cloudformation describe-stack-resources --profile p2r --stack-name p2r --logical-resource-id s3Bucket | jq -r ".StackResources[0].PhysicalResourceId")
if [ $? -ne 0 ]; then
    echo ${POSTGRES_TO_REDSHIFT_S3_DATABASE_EXPORT_BUCKET}
    exit $?
fi

# Make sure we're trying to empty the right bucket
echo ${POSTGRES_TO_REDSHIFT_S3_DATABASE_EXPORT_BUCKET} | grep -e '^p2r-s3bucket'
if [ $? -ne 0 ]; then
    echo ${POSTGRES_TO_REDSHIFT_S3_DATABASE_EXPORT_BUCKET}
    exit $?
fi
aws s3 rm s3://${POSTGRES_TO_REDSHIFT_S3_DATABASE_EXPORT_BUCKET} --recursive --profile p2r
